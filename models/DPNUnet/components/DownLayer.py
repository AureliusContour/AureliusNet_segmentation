...
class AureliusDownLayer(nn.Module):
	def __init__(self, in_ch, size_conv_a, size_conv_b, size_conv_c, split_size, checkpoint=False):
		"""
		A Down Layer that composes of an initial 3x3 Max Pooling (stride 2),
		followed by a number of DPN Blocks that takes in the arguments:
		1. input channel, 
		...
		6. num of DPN Blocks.
		"""
		super(AureliusDownLayer, self).__init__()
		self.max_pooling = nn.MaxPool2d(2)
		self.dpn = DPNBlock(in_ch, size_conv_a, size_conv_b, size_conv_c, split_size)
		self.checkpoint = checkpoint
	...
	def forward(self, x):
		x = self.max_pooling(x)
		if self.checkpoint:
			x = checkpoint.checkpoint(self.custom_checkpoint_call(self.dpn), x, use_reentrant=False)
		else:
			x = self.dpn(x)
		return x
